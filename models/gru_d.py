...
class Model(nn.Module):
    ...
    def forward(self, data, direct):
        ...
        for t in range(SEQ_LEN):
            ...
            # x_h = m * x + (1 - m) * f * gamma_x
            # x_h = m * x + (1 - m) * ((1 - gamma_x) * f + gamma_x * me)
            # x_h = m * x + (1 - m) * me
            # x_h = m * x + (1 - m) * me * gamma_x
            # x_h = m * x + (1 - m) * k
        ...
        mre = torch.sum(torch.abs(evals - imputations) * eval_masks) / (torch.sum(torch.abs(evals) * eval_masks) + 1e-5)

        mae = torch.sum(torch.abs(evals - imputations) * eval_masks) / (torch.sum(eval_masks))
